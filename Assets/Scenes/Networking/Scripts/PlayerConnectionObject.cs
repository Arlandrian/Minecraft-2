...
public class PlayerConnectionObject : NetworkBehaviour {
    ...
    /*
    [Command]
    public void CmdSynchChunk(string chunkName,Vector3 blockPosition,Block.BlockType newType,List<string> redrawList) {
        Chunk c;
        if(World.chunks.TryGetValue(chunkName, out c)) {
            c.chunkData[(int)blockPosition.x, (int)blockPosition.y,(int)blockPosition.z].bType = newType;
            c.changed = true;
        }
